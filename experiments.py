...
from config.config import nb201_hyper_params_setting, nb201_dataset_list, meta_hyper_params_setting, meta_dataset_list
...
from utils.meta_d2a import MetaTestDataset, MetaSurrogateUnnoisedModel, load_graph_config, load_model
...
def get_topk_archs(x: torch.Tensor, dataset: str, k: int, api): 
    ...
    # 准备元学习预测器    
    test_dataset = MetaTestDataset(data_path='./meta_acc_predictor/data/meta_predictor_dataset/', data_name=dataset, num_sample=20)
    ...
    if len(topk_indices) < k:
        print(f'>>> Only found {len(topk_indices)} unique architectures in the top-{k} architectures.')
...
def exp_with_rand_seed_in_nb201(dataset:str):
    ...
    print(f'>>> In {args['rand_exp_num']} random seed experiment on {dataset} dataset, average max accuracy is {avg_max_acc / args["rand_exp_num"]:.2f}, average duration is {avg_duration / args["rand_exp_num"]:.2f} seconds, average uniqueness rate is {avg_uniq_rate / args["rand_exp_num"]:.2f}.\n')
...
def exp_with_rand_seed_in_meta_predictor(dataset:str):
    ...
    for exp in range(args['rand_exp_num']):
        ...
        x = get_topk_archs(x=x, dataset=dataset, k=args['topk'], api=api)
        max_acc = eval_architectures(
            x=x.cpu(), 
            api=api, 
...
def exp_with_fixed_seed_in_nb201(dataset:str):
    ...
    for seed in seed_list:
        ...
        avg_duration += duration
        avg_uniq_rate += uniq_rate
    print(f'>>> For dataset {dataset}, average search duration is {avg_duration / len(seed_list):.2f} seconds, average uniqueness rate is {avg_uniq_rate / len(seed_list):.2f}.\n')
