...
def neural_predictor(operation_matrix, api, dataset, test_dataset, meta_surrogate_unnoised_model, nasbench201):
...
def meta_arch_fitness(operation_matrix, api, dataset, test_dataset, meta_surrogate_unnoised_model, nasbench201):
    assert dataset in ['cifar10', 'cifar100', 'aircraft', 'pets'], 'Unsupported dataset: {}'.format(dataset)
    pred_acc, valid_rate = neural_predictor(operation_matrix, api, dataset, test_dataset, meta_surrogate_unnoised_model, nasbench201) # 架构的准确率
