...
import copy
import time
from utils.nb201_fitness import arch_fitness, diversity_score
...
def crossover(population:int, x:torch.Tensor, cross_rate:float, eta:float):
    ...
    for individual in range(0, population - 1, 2):
        if torch.rand(size=[1]) <= cross_rate:
            x[individual], x[individual + 1] = crossover_operator(x[individual], x[individual + 1], cross_rate, eta)
    return normalize(x)


def mutate(population:int, x:torch.Tensor, mut_rate:float, eta:float, lower_bound=-1, upper_bound=1):
    ...
    for i in range(population):
        if torch.rand(size=[1]) <= mut_rate:
            ...
            if mu <= 0.5:
                delta = (2 * mu + (1 - 2 * mu) * ((1 - delta1)) ** (eta + 1)) ** (1 / (1 + eta)) - 1
            else:
                delta = 1 - (2 * (1 - mu) + (2 * mu - 1) * ((1 - delta2) ** (eta + 1))) ** (1 / (1 + eta))
...
def roulette_wheel(normalized_fitness, n_elitism, n_diver, x_parent, elite_indices, diver_indices, max_iter_time):
    ...
    for i in range(n_elitism+n_diver, x_parent.shape[0]):
        ...
        while not succ_select:
            ...
            for j in range(len(cum_prob)): 
                if (j not in elite_indices) and (j not in diver_indices) and (j not in selected_indices):
...
def select(x_prev:torch.Tensor, x_next:torch.Tensor, fitness_prev:torch.Tensor, elite_rate:float, diver_rate:float, api, dataset:str, max_iter_time:float, nb201_or_meta:str, test_dataset=None, meta_surrogate_unnoised_model=None, nasbench201=None):
    ...
    if nb201_or_meta == 'nb201':
        _, fitness, _ = arch_fitness(operation_matrix=x, api=api, dataset=dataset)
    elif nb201_or_meta == 'meta':
        _, fitness, _ = meta_arch_fitness(operation_matrix=x, api=api, dataset=dataset, test_dataset=test_dataset, meta_surrogate_unnoised_model=meta_surrogate_unnoised_model, nasbench201=nasbench201)
    ...
    normalized_fitness = (fitness - fitness.min()) / (fitness.max() - fitness.min())   # 避免适应度值过大导致的数值不稳定，或者为负数导致相加出错
    selected_indices = roulette_wheel(normalized_fitness, n_elitism, n_diver, x_prev, elite_indices, diver_indices, max_iter_time)
