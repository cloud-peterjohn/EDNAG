...
def get_pred_acc(arch_igraph, device, test_dataset, meta_surrogate_unnoised_model):
...
def meta_predictor(
        test_dataset, meta_surrogate_unnoised_model,
        arch_igraphs, device='cuda'
        ):
    ...
    with torch.no_grad():     
        for i in range(len(arch_igraphs)):
            y_pred_list[i] = get_pred_acc(arch_igraphs[i], device, test_dataset, meta_surrogate_unnoised_model)
    return y_pred_list
...
def meta_neural_predictor(test_dataset, meta_surrogate_unnoised_model, arch_str_list, dataset_name, nasbench201):
    ...
    Returns:
        y_pred_list: list of predicted accuracies for the input architectures.
    """
    assert dataset_name in ['cifar10', 'cifar100', 'aircraft', 'pets'], f'Unsupported dataset: {dataset_name}'
    ...
    if test_dataset is None:
        test_dataset = MetaTestDataset(data_path='./meta_acc_predictor/data/meta_predictor_dataset/', data_name=dataset_name, num_sample=20)
    ...
    y_pred_list = meta_predictor(
        test_dataset=test_dataset, meta_surrogate_unnoised_model=meta_surrogate_unnoised_model,
        arch_igraphs=arch_igraphs, device='cuda' if torch.cuda.is_available() else 'cpu')
...
def test_meta_predictor():
    ...
    for dataset in ['cifar10', 'cifar100', 'aircraft', 'pets']:
        ...
        meta_surrogate_unnoised_model = load_model(model=meta_surrogate_unnoised_model, ckpt_path='meta_acc_predictor/unnoised_checkpoint.pth.tar')
        print(f'\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\nDataset: {dataset}')
        y_pred_list = np.array(meta_neural_predictor(test_dataset, meta_surrogate_unnoised_model, arch_str_list, dataset, None))
        print(f'Predicted accuracies: {y_pred_list}')
        sorted_indices = np.argsort(y_pred_list)[::-1] + 1 
        print(f'Sorted indices: {sorted_indices}')
        if dataset=='cifar10': print('CIFAR10 acc: [8, 2, 10, 6, 4, 9, 11, 3, 1, 7, 5, 12]')
