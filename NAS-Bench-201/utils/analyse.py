...
def compute_uniqueness(arch_op_matrices:torch.Tensor):
    assert arch_op_matrices.shape[1] == 7 * 8, 'Invalid shape: {}'.format(arch_op_matrices.shape)
    arch_op_matrices = arch_op_matrices.view(arch_op_matrices.shape[0], 8, 7)
    ops_decoder = ['input', 'output', 'none', 'skip_connect', 'nor_conv_1x1', 'nor_conv_3x3', 'avg_pool_3x3']
    valid_arch_str_list = []
