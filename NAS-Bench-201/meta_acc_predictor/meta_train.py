...
class MetaTrainDatabase(Dataset):
  def __init__(self, dataset_name, data_path, num_sample):
    assert dataset_name in ['cifar10', 'cifar100'], f'Invalid dataset name: {dataset_name}, it should be in [cifar10, cifar100, imagenet16-120]'
    self.mode = 'train'
    self.acc_norm = True
    self.num_sample = num_sample
...
def get_meta_train_loader(dataset_name, num_sample, batch_size):
    assert dataset_name in ['cifar10', 'cifar100', 'imagenet-1k'], f'Invalid dataset name: {dataset_name}, it should be in [cifar10, cifar100, imagenet-1k]'
    if dataset_name in ['cifar10', 'cifar100']:
        dataset = MetaTrainDatabase(dataset_name=dataset_name, data_path='meta_acc_predictor/data/meta_predictor_dataset', num_sample=num_sample)
    else:
        dataset = MetaTrainDatabaseImgnet(data_path='meta_acc_predictor/data/meta_predictor_dataset', num_sample=num_sample)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, num_workers=1, collate_fn=collate_fn)
    return loader

def train_meta_predictor(epoches_list, num_sample, batch_size, device='cuda' if torch.cuda.is_available() else 'cpu', checkpoint_path='meta_acc_predictor/unnoised_checkpoint.pth.tar', from_scratch=True): 
    graph_config = load_graph_config(graph_data_name='nasbench201', nvt=7, data_path='meta_acc_predictor/data/nasbench201.pt')
    model = MetaSurrogateUnnoisedModel(nvt=7, hs=512, nz=56, num_sample=20, graph_config=graph_config)
    ...
    for idx in range(len(epoches_list)):
        ...
        loader = get_meta_train_loader(dataset_name=dataset_name, num_sample=num_sample, batch_size=batch_size)
        model.to(device)
        model.train()
...
if __name__ == '__main__':
    ...
    train_meta_predictor(epoches_list=epoches_list, num_sample=20, batch_size=64, from_scratch=False)
