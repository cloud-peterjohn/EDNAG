...
def evo_diff(dataset, api, num_step, population_num, 
             geno_shape, temperature, diver_rate, 
             noise_scale, mutate_rate, elite_rate, 
             mutate_distri_index, seed, plot_results, 
             save_dir, nb201_or_meta, max_iter_time):
    start_time = time.time()
    assert nb201_or_meta == 'nb201', 'nb201_or_meta should be nb201, but got {}'.format(nb201_or_meta)
    ...
    for t, alpha in bar:
        try:
            ...
            # 计算适应度值
            accurancy, fitness, valid_rate = arch_fitness(operation_matrix=x, api=api, dataset=dataset)
            ...
            if t != num_step - 1:
                ...
                x = select(x_prev=x_prev, x_next=x, fitness_prev=fitness, elite_rate=elite_rate, diver_rate=diver_rate, api=api, dataset=dataset, max_iter_time=max_iter_time, nb201_or_meta=nb201_or_meta, test_dataset=None, meta_surrogate_unnoised_model=meta_surrogate_unnoised_model, nasbench201=nasbench201)
                x_prev = copy.deepcopy(x)
            else:
    ...
    if plot_results:
        plot_denoise(save_dir=save_dir, avg_acc_trace=avg_acc_trace, max_acc_trace=max_acc_trace, valid_rate_trace=valid_rate_trace, uniq_rate_trace=uniq_rate_trace, seed=seed, dataset=dataset)
    end_time = time.time()

    return max_acc_trace[-1], end_time - start_time, uniq_rate, x

def evo_diff_meta(dataset, api, num_step, population_num, 
             geno_shape, temperature, diver_rate, 
             noise_scale, mutate_rate, elite_rate, 
             mutate_distri_index, seed, plot_results, 
             save_dir, nb201_or_meta, max_iter_time):
    ...
    for t, alpha in bar:
        try:
            ...
            # 计算适应度值
            accurancy, fitness, valid_rate = meta_arch_fitness(operation_matrix=x, api=api, dataset=dataset, test_dataset=None, meta_surrogate_unnoised_model=meta_surrogate_unnoised_model, nasbench201=nasbench201)
            fitness = mapping_fn(fitness)
            max_acc = accurancy.max().item()
            avg_acc = accurancy.mean().item()
            ...
            if t != num_step - 1:
                ...
                x = mutate(population=population_num, x=x, mut_rate=mutate_rate, eta=math.ceil(mutate_distri_index * 1.5))  # 最后一次迭代，减小变异强度
                x = select(x_prev=x_prev, x_next=x, fitness_prev=fitness, elite_rate=elite_rate, diver_rate=diver_rate, api=api, dataset=dataset, max_iter_time=max_iter_time, nb201_or_meta=nb201_or_meta, test_dataset=None, meta_surrogate_unnoised_model=meta_surrogate_unnoised_model, nasbench201=nasbench201)
                x_prev = copy.deepcopy(x)
